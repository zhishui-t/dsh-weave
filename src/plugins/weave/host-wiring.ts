...
import type { GetStatusInput, SubmitTaskInput } from './cli-mcp.js'
 ...
 *    → 插件侧通过 `registerWeaveMcpTools` 把 `WeaveMcp` 的全部 15 个 P0 业务命令
 *      以 dsh-tools 形状注册，DHS 会话模型即可直接调用。
...
/** 将 WeaveMcp 的全部 15 个 P0 命令映射为 dsh-tools ToolDefinition 列表（t37 7 个 + t36 8 个，t39 补齐）。 */
export function buildWeaveToolDefinitions(mcp: WeaveMcp, options: WeaveHostOptions = {}): HostToolDefinition[] {
  ...
  const defs: HostToolDefinition[] = [
    {
      name: `${prefix}submit_task`,
      description: '提交任务：按描述/项目/版本创建 DAG，经团队校验后写入 tasks/dags/edges',
      parameters: {
      ...
      execute: (args) => mcp.submitTask(args as unknown as SubmitTaskInput),
    },
    {
    ...
    },
    {
      ...
      parameters: { task_id: { type: 'string', minLength: 1 }, feedback: { type: 'string', minLength: 1 } },
      output: { schema: OUTPUT_SCHEMA, render: (args, value) => jsonText(value) },
    ...
    },
    {
      ...
      parameters: { task_id: { type: 'string', minLength: 1 } },
      output: { schema: OUTPUT_SCHEMA, render: (args, value) => jsonText(value) },
    ...
    },
    {
      ...
      parameters: { team_id: { type: 'string', minLength: 1 }, session_id: { type: 'string' } },
      output: { schema: OUTPUT_SCHEMA, render: (args, value) => jsonText(value) },
    ...
    },
    {
      ...
      parameters: { knowledge_id: { type: 'string', minLength: 1 } },
      output: { schema: OUTPUT_SCHEMA, render: (args, value) => jsonText(value) },
    ...
    },
    {
      ...
      parameters: { knowledge_id: { type: 'string', minLength: 1 }, reason: { type: 'string' } },
      output: { schema: OUTPUT_SCHEMA, render: (args, value) => jsonText(value) },
...
export function registerWeaveCommand(
  ...
  const disposer = commands.register({
    ...
    description:
      ...
      'task submit|status|revise|accept|retry|skip|cancel|reopen、dag <dag_id>、' +
