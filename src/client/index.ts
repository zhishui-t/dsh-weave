...
const ROUTES: Array<{ key: Route; label: string; desc: string }> = [
  { key: 'overview', label: '总览', desc: '任务与执行器的整体运行状态，含修订记录。' },
  { key: 'teams', label: '团队', desc: '查看团队并创建新的协作团队。' },
  { key: 'knowledge', label: '知识库', desc: '知识/图谱/转换由 Prism 控制面承载。' },
...
interface SessionStatusData {
  ...
  function OverviewPage({ navigate }: { navigate: (route: Route) => void }) {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        Card({
          title: '知识库（Prism）',
          meta: React.createElement('span', null, '知识/图谱/文档转换由 Prism 控制面承载，点击前往。'),
  ...
  /**
   * 知识/图谱/文档转换已移交 Prism 控制面（内嵌 prism serve，默认 http://127.0.0.1:7777）。
  ...
  function WeaveDashboard({ onClose }: { onClose: () => void }) {
    ...
    const pages: Record<Route, any> = {
      ...
      knowledge: React.createElement(PrismKnowledgePage),
