...
## §额度中断项本地处置补记（T47/T66/T72，2026-08-29 补）

**背景**：T47/T66/T72 均因 GLM 额度在任务台账中停留在 FAILED。经本地核实与补跑，处置如下：

| 任务 | 原失败原因 | 本地处置 |
| --- | --- | --- |
| T47 | 2026-08-28 07:46 错误码 **1308**（5 小时上限） | 目标改动已在工作树落地：三处落点（`CAPTAIN_DISCIPLINE`/README/doc/05 §7）均为团队无关措辞，无“长安”硬编码；host-wiring.test + typecheck 通过 |
| T66 | 2026-08-28 16:10 错误码 **1310**（每周/每月上限） | 收口补齐：§总收口已追加，全量门禁 617/39 + e2e 54/54 + typecheck/build/lint 0 error，总提交 `e626b14` |
| T72 | 2026-08-28 16:10 错误码 **1310**（每周/每月上限） | 本地探针验证：`intentApplied.thought = { requested:max, effective:max, supported:true }`，说明 thoughtLevel 传递/扩展协商/`session/setThoughtLevel` 应用链正常；真实模型 turn 仍被 1310 拒绝，未发现需修复的代码缺口 |

**说明**：上述任务在任务台账中已按本地完成处置更新为 COMPLETED；若额度恢复后仍需真实模型回读，可再补跑 T72 的最终探针。
