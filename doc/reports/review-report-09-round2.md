# QA 审核报告：doc/design/09-图谱全量实施设计.md（第 2 轮）

> 审核人：qa（ds梁子）
> 审核对象：`doc/design/09-图谱全量实施设计.md`（第 2 轮修订版）
> 上一轮结论：`doc/reports/review-report-09-round1.md`（P0-1~P0-6 不通过）
...
- 建议改法：§4 改为按 code/knowledge/obsidian/document 分组，每组 2~3 条「前置条件→操作→期望」负向用例（未构建、无匹配 node、affected 空数组、超白名单/超大小、冲突三分支、job 不存在、超时/取消）；T8 内容写明 spec 文件（如 `e2e/features/graph-negative.spec.ts` 等）与 mock 端点清单。
