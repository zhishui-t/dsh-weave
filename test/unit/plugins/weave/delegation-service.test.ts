...
} from '../../../../src/plugins/weave/host/settings-store'
...
import {
  ...
  type TaskContext,
  type TeamConfigLike,
} from '../../../../src/plugins/weave/scheduling/delegation-service'
import { ExecutorProviderRegistry } from '../../../../src/plugins/weave/executors/executor-provider'
  ...
  it('成功路径：四类执行器均走同一 start 入口（spawn/fork/codex/zcode），参数与输出结构正确', async () => {
    for (const executor of ['spawn', 'fork', 'codex', 'zcode']) {
      ...
      expect(knowledge.calls).toEqual([
        { taskId: 'task-1', projectId: 'proj-weave', version: 'v0.2.0', roleId: 'coder', keywords: BASE_TASK.description, limit: BASE_TEAM.knowledge_injection, slim: true },
      ])
    }
  })
