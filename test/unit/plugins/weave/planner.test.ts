...
import { TeamPlanner, assertAcyclic, createPlanTasksHandler, type PlanTasksInput } from '../../../../src/plugins/weave/scheduling/planner'
...
describe('TeamPlanner.plan', () => {
  it('按依赖落库三表：无依赖 WAITING、有依赖 BLOCKED，边与依赖一致', async () => {
    ...
    const dag = await new (await import('../../../../src/plugins/weave/dag/repository')).DagRepository(persistence).loadDag(output.dag_id)
...
describe('TeamPlanner.plan 追加模式（doc/05 §6.1 P1-A）', () => {
  it('正常追加：编号 DAG 域递增 T3/T4、appended=true、tasks 仅含新增、落库合并为 4 任务', async () => {
    ...
    const dag = await new (await import('../../../../src/plugins/weave/dag/repository')).DagRepository(persistence).loadDag(first.dag_id)
