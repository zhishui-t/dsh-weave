...
import { WeaveCli } from '../../../../src/plugins/weave/host/cli-mcp'
...
async function newEnv(registry?: ExecutorRegistry, statusNotifier?: TaskStatusNotifier, audit?: AuditLog): Promise<Env> {
  ...
  const mcp = new WeaveMcp({
    ...
    statusNotifier,
    audit,
    teamManager: new (await import('../../../../src/plugins/weave/team/team-manager')).TeamManager(registry2, { teamsDir: rootDir, persistence: p }),
...
describe('WeaveMcp（MCP Tool 层，治理面）', () => {
  ...
  it('executionHooks.cancelTask / resumeTask：taskCancel 与 taskRetry 触发联动', async () => {
    ...
    const hookMcp = new WeaveMcp({
      persistence: p,
      teamManager: new (await import('../../../../src/plugins/weave/team/team-manager')).TeamManager(new ExecutorRegistry(), { teamsDir: '/nonexistent', persistence: p }),
...
describe('WeaveCli（/weave 命令）', () => {
  ...
  it('task revise/accept：保温期命令文本输出', async () => {
    ...
    const router = new (await import('../../../../src/plugins/weave/scheduling/feedback-router')).FeedbackRouter({
      tasks: p.tasks,
      feedback: p.feedback,
      sessionTracker: new SessionTracker(p.feedback),
    })
...
describe('WeaveMcp 补充：知识审核 / 任务运维 / 禁令列表（t36）', () => {
  ...
  it('配置缺失：无 knowledgeReview/circuitBreaker 时 configuration_error', async () => {
    ...
    const mcp = new WeaveMcp({
      persistence: p,
      teamManager: new (await import('../../../../src/plugins/weave/team/team-manager')).TeamManager(registry, { teamsDir: rootDir, persistence: p }),
