...
describe('WeaveScheduler（DAG 依赖调度）', () => {
  ...
  it('知识审核闭环：收敛时候选 >0 提醒队长审核', async () => {
    ...
    await waitUntil(async () => notices.some((n) => n.text.includes('沉淀待审')))
    expect(notices.some((n) => n.text.includes('/weave knowledge review'))).toBe(true)
    expect(notices.some((n) => n.text.includes('/weave knowledge approve'))).toBe(true)
  ...
  it('知识审核闭环：候选计数异常仅告警，不阻断收敛通知', async () => {
    ...
    expect(notices.some((n) => n.text.includes('沉淀待审'))).toBe(false)
